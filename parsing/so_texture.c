...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/09/29 21:45:02 by mbarhoun          #+#    #+#             */
/*   Updated: 2025/09/30 16:45:08 by mbarhoun         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../cub3d.h"

static int	check_so(t_config *config, char *line, int r, bool *flag)
{
	if (line[r] == 'S' && line[r + 1] == 'O' && is_all_space(line[r + 2]))
	{
		if (*flag == 1)
		...
		if (!line[r])
			return (p_error("Config not valid\n"), 0);
		config->txt->so_texture = ft_strdup(line + r);
		if (!config->txt->so_texture)
...
static int	check_rsl(t_config *config, t_var *var, t_file **file, t_file **tmp)
{
	...
	if (real_char_len((*tmp)->line) > 1)
		count++;
	res = check_so(config, (*tmp)->line, var->r, &var->flag);
	if (!res)
		return (0);
	else if (res == 2)
	{
